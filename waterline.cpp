...
#include "qpoint.h"
#include <QDebug>
WaterLine::WaterLine(QWidget *parent) : QWidget(parent)
{
...
void WaterLine::paintEvent(QPaintEvent *){
//    startPoint = QPoint(this->geometry().x()+20,this->geometry().y()+30);
    A=startPoint;
...
void WaterLine::setPersent(int persent){
    this->persent = persent;
    this->repaint();
}
WaterLine::WaterLine(int width,int height,int child_width,int child_height){
//    resize(800,600);
    setWindowTitle(tr("监控"));
...
void WaterLine::setHeight(int height){
    this->height = height;
}
void WaterLine::setWidth(int width){
    this->width = width;
}
void WaterLine::setStartPoint(QPoint *start){
    this->startPoint=QPoint( start->x(),start->y());
}
